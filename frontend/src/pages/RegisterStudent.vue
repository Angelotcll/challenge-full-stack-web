<template>
  <v-container>
    <v-form ref="form" v-model="valid">
      ...
      <v-row>
        <v-col>
          <v-btn :disabled="!valid" @click="submitForm" color="#53B257">{{
            isEditing ? "Atualizar" : "Salvar"
          }}</v-btn>
        </v-col>
        <v-col>
          <v-btn @click="cancelForm" color="red">Cancelar</v-btn>
        </v-col>
      </v-row>
    </v-form>
...
const nameRules = [
  (v: string) =>
    /^[a-zA-Z]{2,}[ ][a-zA-Z]{2,}$/.test(v) || "Informe nome e sobrenome",
];
const emailRules = [
  (v: string) =>
...
const raRules = [
  (v: string) =>
    /^[0-9]{6,6}$/.test(v) ||
    "Registro acadêmico é obrigatório, insira somente números",
];
const cpfRules = [
  (v: string) =>
...
const formatCpf = () => {
  ...
  if (value.length <= 3) {
    ...
    student.value.cpf = value.replace(
      /(\d{3})(\d{3})(\d{3})(\d{0,2})/,
      "$1.$2.$3-$4"
    );
  }
