...
export class StudentService {
  ...
  async create(createStudentDto: CreateStudentDto): Promise<Student> {
    ...
    if (existingStudent)
      throw new ConflictException(
        'Aluno com esse registro acadêmico já existe',
      );
  ...
  async findOne(id: UUID): Promise<Student> | null {
    ...
    if (!response)
      throw new NotFoundException(`Aluno com ${id} não encontrado`);
    return response;
  }
  ...
  async update(id: UUID, updateStudentDto: UpdateStudentDto) {
    ...
    if (!student)
      throw new NotFoundException(`Aluno com o ${id} não encontrado`);
